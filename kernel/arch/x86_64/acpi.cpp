...
namespace acpi {
    ...
    static void parsemadt(void) {
        ...
        memset(apic::processors, 0, sizeof(uint8_t) * ACPI_MAX_PROCESSORS);
    ...
    void initialise(void) {
        ...
        if(mboot::info.rsdp == nullptr) panic("system is non-ACPI compliant!");

        // Calculate regular and extended checksums for the RSDP.
        uint8_t c1 = checksum((char*) mboot::info.rsdp, 20);
        uint8_t c2 = checksum((char*) mboot::info.rsdp, 36);
        if(c1 != 0) panic("RSDP table checksum not valid! 0x%hhx != 0x00.", c1);
        if(mboot::info.rsdp->revision == 2 && c2 != 0) panic("RSDP extended table checksum not valid! 0x%hhx != 0x00.", c2);

        // Make sure to set the correct system descriptor table address in the ACPI descriptor.
        descriptor.msdt = (mboot::info.rsdp->revision == 0) ? mboot::info.rsdp->rsdtaddr : mboot::info.rsdp->xsdtaddr;
        descriptor.msdt = (uintptr_t) mem::allocatemmio(descriptor.msdt, 1);
        descriptor.revision = mboot::info.rsdp->revision;
